...
const materials = ["Italian leather","Brushed steel","Silk blend","Organic cotton","Polished acetate","Suede","Vegan leather","Sterling silver","Cashmere blend","Titanium"];
const colors = ["Noir","Ivory","Espresso","Champagne","Midnight","Sable","Blush","Slate","Camel","Forest"];
const editions = ["Classic","Signature","Édition","Studio","Heritage","Atelier No.","Reserve","Capsule","Essential","Limited"];
...
export const products: Product[] = Object.entries(names).flatMap(([cat, arr]) =>
  Array.from({ length: PRODUCTS_PER_CATEGORY }, (_, i) => {
    ...
    const pass = Math.floor(i / arr.length);
    const name = pass === 0 ? baseName : `${baseName} ${editions[pass % editions.length]} ${pass}`;
    return {
      id: `${cat}-${i+1}`,
      name,
      slug: `${cat}-${name.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/-$/,"")}-${i+1}`,
      sku: `AUR-${cat.slice(0,3).toUpperCase()}-${String(i+1).padStart(4,"0")}`,
      description: `A considered ${cat.slice(0,-1)} silhouette designed for modern international wardrobes.`,
      priceCents: (basePrice[cat] + i * 7) * 100,
